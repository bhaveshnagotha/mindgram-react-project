...
export interface IBoxStepperData {
  boxComponent: any
  date: string
}

interface IVerticalTimelineProps {
  data: IBoxStepperData[]
  activeIndex: number
  title?: string
  markerSize?: number
  markerBorder?: number
...
const VerticalBoxedTimeline = ({
  ...
  return (
    <StepperContainer size={size} markerSize={markerSize}>
      {title && <h2>{title}</h2>}
      <Stepper>
        {data?.map((step: IBoxStepperData, index: number) => {
          ...
          return (
            <StepperStep key={index}>
              ...
              />
              <StepperInfoWrapper>{step.boxComponent}</StepperInfoWrapper>
            </StepperStep>
          )
        })}
      </Stepper>
...
export default VerticalBoxedTimeline
